...
import { createBancoAula, updateBancoAula } from '@/services/banco'
...
export function BancoAulaModal({ turmaId, turmaIconColor, banco, currentUser, onClose, onSaved }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      if (isCreate) {
        await createBancoAula(turmaId, payload, currentUser!)
      } else {
        await updateBancoAula(turmaId, banco!.id, payload, currentUser!.role)
      }
    } catch (e) {
      setSaving(false)
