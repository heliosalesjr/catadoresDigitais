...
import { createAula, updateAula, deleteAula } from '@/services/aulas'
...
export function AulaModal({
  ...
  async function handleSave() {
    ...
    try {
      if (isCreate) {
        await createAula(turmaId, payload, user!.role)
      } else {
        await updateAula(turmaId, aula!.id, payload, user!.role)
      }
    } catch (e) {
      setSaving(false)
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    await deleteAula(turmaId, aula.id).catch(console.error)
    setDeleting(false)
    onSaved()
  }
  ...
  async function handleApprove() {
    if (!aula) return
    setSaving(true)
    await updateAula(turmaId, aula.id, { status: 'published' }, user!.role).catch(console.error)
    setSaving(false)
    onSaved()
  }
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col gap-5 p-5 overflow-y-auto">
          ...
          {mode === 'view' && aula && (
            <Link
              to={`/dashboard/aula/${turmaId}/${aula.id}`}
