...
import { createTurma, updateTurma, deleteTurma } from '@/services/turmas'
...
export function TurmaForm({ mode, turmaId, initialData, archived: initialArchived, backHref }: Props) {
  ...
  async function toggleArchived() {
    ...
    try {
      await updateTurma(turmaId!, { archived: nextArchived }, user!.uid)
      setArchived(nextArchived)
    } catch (e) {
      console.error('[toggleArchived]', e)
    }
    setArchiving(false)
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit) {
        await updateTurma(turmaId!, payload, user!.uid)
        id = turmaId!
      } else {
        const created = await createTurma({ ...payload, createdBy: user!.uid })
        id = created.id
      }
    } catch (e) {
      setSaving(false)
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={async () => {
                      ...
                      navigate('/dashboard/admin/turmas')
                    }}
                    disabled={deleting}
