...
import { getTurmaStudents } from '@/services/turmas'
import { listRespostas, updateAula } from '@/services/aulas'
...
export function ChamadaEditModal({ turmaId, turmaIconColor, aula, students, onClose, onChange }: Props) {
  ...
  async function setStatus(email: string, status: Exclude<AttendanceStatus, null>) {
    ...
    await updateAula(turmaId, aula.id, { attendance: updated }, user!.role).catch(console.error)
    setSavingEmail(null)
    onChange()
  }
