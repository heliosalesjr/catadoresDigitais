...
import { updateAula, getMyResposta } from '@/services/aulas'
import { listTeachers } from '@/services/users'
import { updateTurma } from '@/services/turmas'
...
export function ConteudoPanel({ turma, aulas, selectedMonth, canEdit, currentUser, onRefresh, onRefreshTurma, initialTab, isMobile }: Props) {
  ...
  async function submitMaterial(aula: Aula) {
    ...
    await updateAula(turma.id, aula.id, { driveLinks: newLinks }, currentUser!.role).catch(console.error)
    setAdding(null)
    onRefresh()
  }
...
function AulaCard({
  ...
  useEffect(() => {
    ...
    getMyResposta(turma.id, aula.id, currentUser!.email)
  ...
  async function saveAvaliacao(data: Omit<Avaliacao, 'id' | 'createdAt'>) {
    ...
    await updateAula(turma.id, aula.id, { avaliacoes: [...avaliacoes, newAv] }, currentUser!.role).catch(console.error)
  ...
  async function deleteAvaliacao(id: string) {
    ...
    await updateAula(turma.id, aula.id, { avaliacoes: avaliacoes.filter((a) => a.id !== id) }, currentUser!.role).catch(console.error)
    onRefresh()
  }

  async function deleteMaterial(index: number) {
    const updated = aula.driveLinks.filter((_, i) => i !== index)
    await updateAula(turma.id, aula.id, { driveLinks: updated }, currentUser!.role).catch(console.error)
    onRefresh()
  }
  ...
  return (
    ...
    >
      ...
      {canEdit ? (
        ...
        <Link
          to={`/dashboard/aula/${turma.id}/${aula.id}`}
...
function PresencaAulaRow({ aula, turma, canEdit, studentList, todayISO, onRefresh }: PresencaAulaRowProps) {
  ...
  async function handleRegenCode() {
    ...
    await updateAula(turma.id, aula.id, { attendanceCode: newCode }, authUser!.role).catch(console.error)
...
function ProfessoresPanel({ turma, currentUser, onRefresh }: ProfessoresPanelProps) {
  ...
  async function openPicker() {
    ...
    setLoadingDb(true)
    try {
      setDbTeachers(await listTeachers())
    } catch (e) {
  ...
  async function addProfessor(t: DbTeacher) {
    ...
    setSaving(true)
    await updateTurma(turma.id, { professors: [...professors, newProf] }, currentUser!.uid).catch(console.error)
    setSaving(false)
    setShowPicker(false)
    onRefresh()
  }
  ...
  async function removeProfessor(uid: string) {
    ...
    await updateTurma(turma.id, { professors: professors.filter((p) => p.uid !== uid) }, currentUser!.uid).catch(console.error)
    onRefresh()
  }
