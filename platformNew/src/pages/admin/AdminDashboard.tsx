...
import { useUpcomingAulas } from '@/hooks/useTurmas'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat) => (
            <button
              ...
              className="card card-hover p-6 text-left disabled:cursor-default group cursor-pointer"
            >
              ...
              {pendingAulas.length > 0 && (
                <div>
                  ...
                  {pendingAulas.map((aula) => (
                    <Link
                      key={aula.id}
                      to={`/dashboard/turmas/${aula.turmaId}`}
        ...
        <div className="card overflow-hidden">
          ...
          {!turmasLoading && activeTurmas.length === 0 ? (
            <div className="px-6 py-5 border-b text-sm" style={{ borderColor: 'var(--c-border)', color: 'var(--c-subtle)' }}>
              ...
              <Link to="/dashboard/admin/turmas" className="font-semibold underline" style={{ color: 'var(--c-text)' }}>
                Criar turma →
              </Link>
            </div>
          ) : (
            <form onSubmit={handleAddToAllowlist} className="px-6 py-4 flex flex-col sm:flex-row gap-2 border-b" style={{ borderColor: 'var(--c-border)' }}>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-opacity disabled:opacity-50 cursor-pointer"
                ...
                >
                  ...
                  <Tooltip label="Remover da lista">
                    <button
                      ...
                      className="w-8 h-8 rounded-lg flex items-center justify-center border transition-colors flex-shrink-0 disabled:opacity-50 cursor-pointer"
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-6 py-3 border-t" style={{ borderColor: 'var(--c-border)' }}>
                <button
                  ...
                  disabled={page === 1}
                  className="text-xs px-3 py-1.5 rounded-lg border transition-colors disabled:opacity-30 cursor-pointer"
