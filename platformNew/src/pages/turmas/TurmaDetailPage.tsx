...
import { listAulas } from '@/services/aulas'
...
export default function TurmaPage() {
  ...
  const fetchAulas = useCallback(async () => {
    try {
      setAulas(await listAulas(id))
    } catch (e) {
      console.error('[fetchAulas]', e)
    }
  }, [id])
  ...
  const isArchived = turma.archived === true || isTurmaExpired(turma.endDate)
  const canView = isAdmin || user?.role === 'teacher'
  const canEdit = isAdmin || (user?.role === 'teacher' && !isArchived)
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <Tooltip label="Voltar">
            <Link
              to={isAdmin ? '/dashboard/admin/turmas' : '/dashboard'}
              aria-label="Voltar"
        ...
        {canView && (
          <div className="flex items-center gap-2 flex-shrink-0">
            <Tooltip label="Relatório da turma">
              <Link
                to={`/dashboard/admin/turmas/${id}/relatorio`}
            ...
            {isAdmin && (
              <Tooltip label="Editar turma">
                <Link
                  to={`/dashboard/admin/turmas/${id}/editar`}
