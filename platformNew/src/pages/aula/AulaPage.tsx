...
import { getTurma } from '@/services/turmas'
import { getAula, getMyResposta, submitRespostas, submitChamada as submitChamadaService } from '@/services/aulas'
...
export default function ModoAulaPage() {
  const { turmaId, aulaId } = useParams<{ turmaId: string; aulaId: string }>() as { turmaId: string; aulaId: string }
  const { user } = useAuth()
  ...
  useEffect(() => {
    async function load() {
      const [tr, ar, rr] = await Promise.all([
        getTurma(turmaId).catch(() => null),
        getAula(turmaId, aulaId).catch(() => null),
        user?.role === 'student'
          ? getMyResposta(turmaId, aulaId, user.email).catch(() => null)
          : Promise.resolve(null),
      ])
      if (tr) setTurma(tr)
      if (ar) setAula(ar)
      if (rr?.answers) {
        setAnswers(rr.answers)
  ...
  async function submitChamada() {
    ...
    try {
      await submitChamadaService(turmaId, aulaId, chamadaCode.trim(), user!)
      setChamadaState('ok')
      const ar = await getAula(turmaId, aulaId).catch(() => null)
      if (ar) setAula(ar)
    } catch (e) {
      setChamadaError(e instanceof Error ? e.message : 'Erro ao registrar presença.')
  ...
  async function submitAvaliacoes() {
    ...
    setAvalError(null)
    try {
      await submitRespostas(turmaId, aulaId, answers, user!)
